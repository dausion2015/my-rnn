...
#         logits = tf.reshape(logits, [self.batch_size, self.num_steps, self.num_words])
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.Y, [-1,1]),logits=logits)#yreshape后是batchsize*timesteps列1行logits是batchsize*timesteps ，num_words 而groundtruth中每一个字都对应着numwords个概率值
